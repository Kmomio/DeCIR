#!/usr/bin/env python
"""
Rerank FashionIQ retrieval results using Qwen3-VL MLLM.

FashionIQ has dual captions that are combined for reranking.

Usage:
    python scripts/reranking/rerank_fashioniq.py \
        --data_root data/fashioniq \
        --predictions outputs/fashioniq_dress/predictions.json \
        --output_dir outputs/fashioniq_dress_reranked \
        --category dress \
        --split test \
        --grid_k 4 \
...
def create_grid_image(
    ...
    """Create a grid visualization of top-K candidates for FashionIQ.
...
def create_rerank_prompt(captions: List[str]) -> str:
    """Create prompt for FashionIQ MLLM reranking.

    FashionIQ has dual captions that are combined.

    Args:
        captions: List of modification captions (usually 2)

    Returns:
        Prompt string
    """
    # Combine captions with "and"
    combined_caption = " and ".join(captions)

    prompt = f'''I want you help me do the composed image retrieval. Given the reference image (first image) and modified text: "{combined_caption}".
...
def generate_ranking(
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a helpful assistant for fashion image retrieval tasks."
        },
        {
            "role": "user",
            "content": [
...
def main():
    parser = argparse.ArgumentParser(description="Rerank FashionIQ results using Qwen3-VL")
    ...
    print("="*60)
    print("FashionIQ Reranking with Qwen3-VL")
    print("="*60)
    ...
    # Load annotations
    print("\n[1/5] Loading FashionIQ data...")
    anno_file = data_root / 'captions' / f'cap.{args.category}.{args.split}.json'
    ...
    img_name_to_path = {img.stem: img for img in image_files}

    print(f"Found {len(img_name_to_path)} images")
    ...
    for idx, item in enumerate(tqdm(annotations, desc="Reranking")):
        query_id = str(idx)
        ...
        if len(captions) != 2:
            ...
            if query_id in predictions:
                reranked_predictions[query_id] = predictions[query_id]
            continue
        ...
        ref_img_path = image_dir / f"{candidate_name}.jpg"
        if not ref_img_path.exists():
            ref_img_path = image_dir / f"{candidate_name}.png"

        if not ref_img_path.exists():
            print(f"Warning: Reference image not found: {candidate_name}")
        ...
        try:
            # Create grid image
            grid_path = grid_dir / f"fashioniq_{args.category}_{query_id}_grid.jpg"
            if not grid_path.exists():
                create_grid_image(candidates, img_name_to_path, str(grid_path), args.grid_k)

            # Generate reranking with dual captions
            prompt_text = create_rerank_prompt(captions)
